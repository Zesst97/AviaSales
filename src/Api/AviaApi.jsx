...
export default class AviaApi {
  ...
  getSearchId = async (dispatch) => {
    try {
      ...
      if (!response.ok) {
        const message = response.status >= 400 && response.status < 500 ? 'Клиентская ошибка при получении ID' : response.statusText;
  ...
  getTickets = async (searchId, dispatch) => {
    try {
      ...
      if (!response.ok && response.status !== 500) {
        const message = response.status >= 400 && response.status < 500 ? 'Клиентская ошибка при получении билетов' : response.statusText;
        const error = new Error(message);
        error.code = response.status;
        dispatch(setError({ code: error.code, message: error.message }));
        throw error;
